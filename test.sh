...
assert "InclusiveRange2" "$(InclusiveRange 0 5 | stream.mkString , [ ])" "[0,1,2,3,4,5]"
...
assert "takeRightWhile" "$(Chars hello | (F(){ [[ "$1" != "e" ]]; }; stream.takeRightWhile F) | stream.mkString , [ ])" "[l,l,o]"
assert "dropRightWhile" "$(Chars hello | (F(){ [[ "$1" != "l" ]]; }; stream.dropRightWhile F) | stream.mkString , [ ])" "[h,e,l,l]"
