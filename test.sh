...
assert "orElse" "$(Option asdf | (stream.orElse <(string.println "test")))" "asdf"
assert "orElse" "$(Option | (stream.orElse <(string.println "test")))" "test"
...
assert "zipWith" "$(List foo "bar " test longword | (stream.zipWith <(Chars "$1" | stream.length)) | stream.mkString , [ ])" "[foo 3,bar  4,test 4,longword 8]"
...
assert "prependAll" "$(List hello | (stream.prependAll <(string.println "start")) | stream.mkString , [ ])" "[start,hello]"
assert "appendAll" "$(List hello | (stream.appendAll <(string.println "end")) | stream.mkString , [ ])" "[hello,end]"
assert "concat" "$(List hello | (stream.concat <(string.println "end")) | stream.mkString , [ ])" "[hello,end]"
